bayeos.getReferences <- function(id, con = 1) {
  ...
  res = append(
    bayeos.call(con, 'ObjektHandler.getDirectReferences', as.integer(id)),
    bayeos.call(
      con,
      'ObjektHandler.getInheritedReferences',
      as.integer(id)
    )
  )
  #	id_ref=sapply(res,function(x) x[[5]])
  type = sapply(res, function(x) {
    if (is.null(x[[1]]))
      '?'
    else
      x[[1]]
  }) #Null for Department - Fix in goat.server?
  name = sapply(res, function(x)
    x[[2]])
  direct = sapply(res, function(x)
    (x[[8]] == id))
  df = data.frame(name, type, direct)
  if (length(df) == 0)
    df = data.frame(name = vector(),
                    type = vector(),
                    direct = vector())
  df
}
...
bayeos.createReferences <- function(id,
                                    to,
                                    tree,
                                    create = FALSE,
                                    id_parent = NULL,
  ...
  res = bayeos.call(
    con,
    'ObjektHandler.createReference',
    as.integer(to),
    as.integer(id),
    as.character(classes[tree])
  )
  bayeos.getReferences(id, con)
}


bayeos.createReferenceNode <- function(name,
                                       tree,
                                       id_parent = NULL,
                                       con = 1) {
  ...
  if (is.null(id_parent))
    id_parent = .localenv$c[[con]]$root_ids[classes[tree]]
  bayeos.call(con,
              'TreeHandler.newNode',
              as.vector(classes[tree]),
              name,
              as.integer(id_parent))[[3]]
}
