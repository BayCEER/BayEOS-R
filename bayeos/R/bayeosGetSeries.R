...
bayeos.deleteRows <- function(ids,
                              datetimes = NULL,
                              from = NULL,
                              until = NULL,
                              con = 1) {
  ...
  for (id in ids) {
    ...
    if (!is.null(from) && !is.null(until)) {
      ...
      res = bayeos.call(con,
                        'MassenTableHandler.removeRowsByInterval',
                        id,
                        timefilter[1],
                        timefilter[2])
    }
...
bayeos.getSeries <- function(ids,
  ...
  if (!localTime) {
    ...
    setBayeosConTZ(con, Sys.timezone())
  ...
  if (is.na(.localenv$c[[con]]$aggfunc[aggfunc]) |
      is.na(.localenv$c[[con]]$aggint[aggint]))
    res = bayeos.call(con, 'MessungHandler.getResolution', ids)
  else
    res = .localenv$c[[con]]$aggint_sec[aggint]
  
  # Check Row Count
  if (!isValidRowCount(res, timefilter, maxrows))
    return(NA)
  
  if (is.na(.localenv$c[[con]]$aggfunc[aggfunc]) |
      is.na(.localenv$c[[con]]$aggint[aggint])) {
    massendaten = bayeos.call(con,
                              'MassenTableHandler.getRows',
                              ids,
                              timefilter,
                              as.integer(statusFilter))[[2]]
    ...
    aggregierte_daten = bayeos.call(con,
                                    'AggregationTableHandler.getRows',
                                    ids,
                                    timefilter,
                                    as.integer(c(
                                      .localenv$c[[con]]$aggfunc[aggfunc], .localenv$c[[con]]$aggint[aggint]
                                    )))
    datetime = as.POSIXct(sapply(aggregierte_daten[[2]], function(x)
      x[[1]]),
      origin = '1970-01-01',
      tz = 'GMT')
    attr(datetime, 'tzone') = .localenv$c[[con]]$tz
    ...
    if (csFlag)
      df$count = sapply(aggregierte_daten[[2]], function(x)
        x[[3]])
  }
  df = zoo(df, datetime)
  attr(df, 'ids') <- ids
...
bayeos.df2zoo <- function(data) {
  is_datetime = sapply(data, function(x)
    any(class(x) == 'POSIXct'))
...
bayeos.zoo2bin <- function(id, data) {
  ...
  as.vector(rbind(
    matrix(writeBin(
      as.integer(id), raw(), size = 4, endian = 'big'
    ), 4, l),
    matrix(writeBin(
      as.vector(rbind(dt, dt2, dt3, dt4)),
      raw(),
      size = 2,
      endian = 'big'
    ), 8, l),
    matrix(writeBin(
      as.numeric(coredata(data)),
      raw(),
      size = 4,
      endian = 'big'
    ), 4, l)
...
bayeos.writeSeries <- function(ids,
                               data,
                               con = 1,
                               update = FALSE,
                               status = NA,
  ...
  for (from in seq(1, nrows, rowstep)) {
    ...
    if (until >= from) {
      ...
      bin = as.vector(unlist(sapply(index, function(i)
        bayeos.zoo2bin(ids[i], data[from:until, i]))))
...
bayeos.getMatrix <- function(ids,
  ...
  if (!isValidRowCount(sec, timefilter, maxrows))
    return(NA)
  if (is.na(.localenv$c[[con]]$aggfunc[aggfunc]) |
      is.na(.localenv$c[[con]]$aggint[aggint]))
    vec = bayeos.call(
      con,
      'MassenTableHandler.getMatrix',
      as.integer(ids),
      timefilter,
      as.integer(statusFilter),
      csFlag
    )
  else
    vec = bayeos.call(con,
                      'AggregationTableHandler.getMatrix',
                      ids,
                      timefilter,
                      as.integer(c(
                        .localenv$c[[con]]$aggfunc[aggfunc], .localenv$c[[con]]$aggint[aggint]
  ...
  bytes = length(vec[[2]])
  if (bytes == 0)
    vec[[2]] = as.raw(NULL) #avoid error for readBin
  ...
  m = zoo(m, datetime)
  if (csFlag)
    names(m) = vec[[1]][sapply(1:colCount, function(x) {
      if (x <= (colCount / 2))
        x * 2 - 1
      else
        (x - colCount / 2)
...
bayeos.import <- function (data,
  ...
  child_class = sapply(.localenv$c[[con]]$childNodes, function(x)
    x$class)
  child_ids = sapply(.localenv$c[[con]]$childNodes, function(x)
    x$id)[child_class == 'messung_massendaten']
  child_names = sapply(.localenv$c[[con]]$childNodes, function(x)
    x$name)[child_class == 'messung_massendaten']
...
bayeos.readCSV <- function(file,
                           tz = 'Etc/GMT-1',
                           dateformat = c('%d.%m.%Y %H:%M'),
                           skip = 0,
                           sep = ';',
                           dec = '.',
                           na.strings = 'NA',
