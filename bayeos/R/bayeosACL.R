...
bayeos.createACL <- function(rid,
  ...
  if (ls)
    bayeos.getACL(id, ls = TRUE)
  res
}

bayeos.deleteACL <- function(rid,
                             id = NA,
                             ls = FALSE,
                             con = 1) {
  id = bayeos.id2id(id, con)
...
bayeos.updateACL <- function(right,
                             value,
                             rid,
                             id = NA,
                             ls = FALSE,
...
bayeos.getRoles <- function(con = 1) {
  if (is.null(.localenv$c[[con]]$roles)) {
    res = c(
      bayeos.call(con, 'LookUpTableHandler.getBenutzer'),
...
bayeos.getRid <- function(name, con = 1) {
  ...
  if (length(id) == 0)
    id = roles$rid[roles$fullname == name]
  if (length(id) == 0)
    id = NA
  id
}
