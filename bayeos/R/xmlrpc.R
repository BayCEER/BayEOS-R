...
createBody =
  function(method, args)
  {
    ...
    sapply(args, function(x)
      newXMLNode("param", rpc.serialize(x), parent = params))
    top
  }

setGeneric("rpc.serialize", function(x, ...)
  standardGeneric("rpc.serialize"))
...
setMethod("rpc.serialize", "NULL", function(x, ...)
  newXMLNode("value", newXMLNode("nil")))

setMethod("rpc.serialize", "POSIXct", function(x, ...) {
  ...
  if (length(x) == 1)
    ...
    newXMLNode("value", newXMLNode("array", newXMLNode(
      "data", sapply(x, function(x)
        newXMLNode(
          "value", newXMLNode(
            "dateTime.iso8601",
...
setMethod("rpc.serialize", "vector", function(x, ...) {
  ...
  if (length(x) == 1)
    ...
    newXMLNode("value", newXMLNode("array", newXMLNode(
      "data", sapply(x, function(x)
        newXMLNode(
          "value", newXMLNode(type, if (type == "string")
            newXMLCDataNode(enc2utf8(x))
            else
              x)
        ))
...
setMethod("rpc.serialize", "list", function(x, ...) {
  if (length(names(x))) {
    newXMLNode("value", newXMLNode("struct", sapply(names(x), function(id) {
    ...
    newXMLNode("value", newXMLNode("array", newXMLNode(
      "data", .children = lapply(x, function(x)
        rpc.serialize(x))
    )))
  }
})

setGeneric('convertToR', function(node)
  standardGeneric('convertToR'))
...
setMethod('convertToR', 'XMLInternalNode', function(node)
{
...
setMethod('convertToR', 'character', function(node)
{
...
xmlRPCToR =
  function(node, ...)
  {
    ...
    switch(
      ...
      'base64' = if (nchar(xmlValue(node)) == 0)
        NULL
      else
        base64(xmlValue(node), encode = FALSE, mode = 'raw'),
...
xmlRPCToR.struct =
  function(node, ...)
  {
    ans = xmlApply(node, function(x)
      xmlRPCToR(x[["value"]][[1]], ...))
    names(ans) = xmlSApply(node, function(x)
      xmlValue(x[["name"]]))
    ans
  }
...
xmlRPCToR.array =
  function(node, ...)
  {
    ans = xmlApply(node[["data"]], function(x)
      xmlRPCToR(x[[1]]))
