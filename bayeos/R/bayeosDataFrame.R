...
bayeos.getDF<-function(ids,rowNr=NULL,con=1){
	...
	if(! is.null(rowNr)){
		...
		if(length(rowNr)==1) rowNr=as.list(rowNr)
	}
	...
	if(length(ids)==1 && bayeos.getNode(ids)$class=='data_frame')
		res=bayeos.call(con,'DataFrameHandler.getFrameRows',as.integer(ids),rowNr) 
	else{
		ids=as.integer(ids)
		if(length(ids)==1) ids=as.list(ids) #force vector over list
...
bayeos.getDFZoo<-function(ids,zooIndexColumn=1,aggregateFun=NULL,con=1){
	checkServerVersion(con,1,9)
	ids=bayeos.id2id(ids,con)
	cnr=0
	for(id in ids){
		...
		if(! is.null(aggregateFun)){
			...
			df=as.data.frame(sapply(df,function(x) aggregate(x~index,FUN=aggregateFun)[,2]))
			index=index_new
		}
		tmp=zoo(df,index)
		names(tmp)=paste(name,names(df))
		if(cnr>0) res=cbind(res,tmp)
		else res=tmp
		cnr=cnr+1
	}
	res
...
bayeos.writeDF<-function(ids,data,rowNr=NULL,con=1){
	checkServerVersion(con,1,9)
	ids=bayeos.id2id(ids,con)
	if(is.null(rowNr)){
	sapply(1:length(ids),function(x){
				if(class(data[[x]])[1]=="factor") data[[x]]=as.character(data[[x]])
				bayeos.call(con,'DataFrameHandler.deleteColValues',as.integer(ids[x])) 
				if(length(data[[x]])==1) #Force vector over list!!
					bayeos.call(con,'DataFrameHandler.writeColValues',as.integer(ids[x]),as.list(data[[x]]))  
				else
				bayeos.call(con,'DataFrameHandler.writeColValues',as.integer(ids[x]),data[[x]]) 
			})
	} else {
		rowNr=as.integer(rowNr)
		if(length(rowNr)==1) rowNr=as.list(rowNr)
		sapply(1:length(ids),function(x){
					if(class(data[[x]])[1]=="factor") data[[x]]=as.character(data[[x]])
					if(length(data[[x]])==1) #Force vector over list!!
						bayeos.call(con,'DataFrameHandler.updateColValues',as.integer(ids[x]),rowNr,as.list(data[[x]]))  
					else
						bayeos.call(con,'DataFrameHandler.updateColValues',as.integer(ids[x]),rowNr,data[[x]]) 
				})
...
bayeos.importDF<-function(id,data,columnSelection=NULL,columnDestination=NULL,append=FALSE,con=1){
	checkServerVersion(con,1,9)
	if(class(data)!='data.frame') return(FALSE);
	...
	ids=sapply(1:length(columnDestination),function(x){
				if(is.na(child_ids[columnDestination[x]]) | is.list(child_ids))
					bayeos.createDFColumn(columnDestination[x],x,.DataType(data[[x]]),con=con,ls=FALSE,id_parent=id)
				else
	...
	if(append){
		cids=ids
		if(length(cids)==1) cids=as.list(cids)
