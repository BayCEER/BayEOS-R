...
checkServerVersion <- function(con, major, minor) {
  if ((.localenv$c[[con]]$bayeosVersion[1] < major) ||
      (.localenv$c[[con]]$bayeosVersion[2] < minor &&
       .localenv$c[[con]]$bayeosVersion[1] == major)) {
    stop(
      paste(
        'The server runs bayeos ',
        paste(.localenv$c[[con]]$bayeosVersion, collapse = '.')
        ,
        '. This function requires bayeos-server >= ',
        major,
        '.',
        minor,
        sep = ''
...
xorEncode <- function(t, key) {
  rt = charToRaw(t)
  if (length(key) < length(rt))
    key = rep(key, ceiling(length(rt) / length(key)) + 1)
...
encode_pwfile <- function(pwFileName, key) {
  warning(
    "Converting old .bayeos.pwd file to new stye with encrypted passwords.
  ...
  pw_file = lapply(pw_file, function(x) {
    if (x[4] > '')
      x[4] = xorEncode(x[4], key)
    x
  })
...
get_interval_from_keyword <- function(interval) {
  ...
  monday = strftime(Sys.Date() - as.POSIXlt(Sys.Date())$wday + 1, format =
                      "%Y-%m-%d %H:%M")
  monday_1 = strftime(Sys.Date() - as.POSIXlt(Sys.Date())$wday - 6, format =
                        "%Y-%m-%d %H:%M")
  first_of_month = strftime(Sys.Date() - as.POSIXlt(Sys.Date())$mday + 1, format =
                              "%Y-%m-%d %H:%M")
  first_of_year = strftime(Sys.Date() - as.POSIXlt(Sys.Date())$yday, format =
                             "%Y-%m-%d %H:%M")
  first_of_month_1 = strftime(as.Date(first_of_month) - as.POSIXlt(as.Date(first_of_month) -
                                                                     1)$mday,
                              format = "%Y-%m-%d %H:%M")
  first_of_year_1 = strftime(as.Date(first_of_year) - as.POSIXlt(as.Date(first_of_year) -
                                                                   1)$yday - 1,
                             format = "%Y-%m-%d %H:%M")
...
setBayeosConTZ <- function(con, tz) {
  if (tz != .localenv$c[[con]]$tz) {
    message(paste(
      'INFO: Setting connection time zone from',
      .localenv$c[[con]]$tz,
      'to',
      tz
    ))
...
isValidRowCount <- function (sec, timefilter, maxrows) {
  if (sec == 0 | is.na(sec))
    return (TRUE)
  estimated_rows = (as.numeric(timefilter[2]) - as.numeric(timefilter[1])) /
    sec
  if (estimated_rows > maxrows) {
    warning(
      paste(
        ...
        'exceeds the maxrows',
        maxrows,
        '. Increase maxrows in the function call or use a different time filter.'
      )
    )
    return(FALSE)
  } else {
...
getTimeFilter <- function (from, until, interval, con) {
  ...
  timefilter = as.POSIXct(c(
    as.POSIXct(timefilter[1], tz = .localenv$c[[con]]$tz),
    as.POSIXct(timefilter[2], tz = .localenv$c[[con]]$tz) - 1
...
listlist2df <- function(ll,
  ...
  cl = lapply(1:length(ll[[1]]), function(x) {
    i = x
